...
class LoginAction extends Action {
	...
	public function checkReset()
	{
		...
		if(!$user)
		{
			...
			$url='http://'.$_SERVER['SERVER_NAME'].$_SERVER['SCRIPT_NAME'].'/Login/resetPass?username='.$usename.'&token='.$token;
			$time = date('Y-m-d H:i');
	...
	public function testLocation()
	{
		...
		$loca = $utils->GetIpLookup("218.28.20.163");
		dump($loca);
		dump($loca['country']." ".$loca['province']." ".$loca['city']);
